...
def parse_num_arg(args, data_list):
    '''Handles numeric range arguments passed to list tag'''
    if not 'num' in args:
        return data_list
    num = args['num'].split(':', 1) 
    if len(num) == 1:
        return data_list[:abs(int(num[0]))]
    elif len(num) == 2:
        start = abs(int(num[0]))
        end = abs(int(num[1]))
    else:
        sys.exit('Zap! Bad argument "{0}"'
        ' at template tag!'.format(args['num'])) 
    if start and end:
...
def tag_list(env, page, args, tpl):
    ...
    if not 'src' in args:
        args['src'] = 'pages'
    # copy the list determined by src parameter
    try:
        data_list = list(env[args['src']].copy().values())
    except KeyError:
        sys.exit('Zap! Bad argument "{0}"'
        ' at template tag!'.format(args['src']))
    # empty list, return
    if not data_list:
        return ''
    # limit the category first
    if 'cat' in args:
        cat_func = lambda c: c['path'].startswith(args['cat'])
        data_list = list(filter(cat_func, data_list))
    # listing order before number of objects
    if args.get('ord') == 'desc':
        data_list.reverse()
    # limit number of objects to show
    try:
        data_list = parse_num_arg(args, data_list)
    except ValueError:
        sys.exit('Zap! Bad argument "{0}"'
        ' at template tag!'.format(args['num']))
