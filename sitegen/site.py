...
class SiteGenerator:
    ...
    def read_page_tree(self, path, parent_page=None):
        ...
        page_data = self._read_page(path)
        page = None
        if page_data:
            page = self._build_page(page_data, parent_page)
            # add page to ordered list of pages
            if not page.is_draft():
                self.pagelist.insert(page)
        for sub_page_path in self._read_subpages_list(path):
            self.read_page_tree(sub_page_path, page or parent_page)
    ...
    def _build_page(self, page_data, parent_page):
    ...
    def _read_subpages_list(self, path):
...
class Site:
    ...
    def generate(self, path):
        ...
        categories = generator._build_categories()

        pages = generator.read_page_tree(f'{path}/data')
        ...
        for page in pages:
            env['page'] = page
            generator._publish_page(page, env)
            print('Generated HTML {!r}.'.format(page.path))
        generator._publish_feeds()
        return pages
